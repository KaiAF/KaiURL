...
a.get('/link/callback', async function(req, res) {
    ...
    fetch(`http://${req.hostname}/api/auth?q=${auth}`, { method: 'get' }).then(async(r) => r.json()).then(async(b) => {
        ...
        if (_URL == "http://localhost") CLIENT_SECRET = process.env.TEST_CLIENT_SECRET_LINK;
        if (_URL == "https://www.kaiurl.xyz") CLIENT_ID = process.env.CLIENT_ID_LINK;
        if (_URL == "https://www.kaiurl.xyz") CLIENT_SECRET = process.env.CLIENT_SECRET_LINK;
                ...
                log.updateOne({ _id: r._id }, {
                    ...
                    user.updateOne({ _id: findUser._id }, { $set: { discord: id } }).then(async() => {
                        await userDiscord.findOne({ userId: id }, async function(e, re) {
                            if (re) {
                                userDiscord.updateOne({ _id: re._id }, {
                                    $set: {
                                        username: username,
                                        userId: id,
